...
def main(args):
    ...
    # generate or load 2D keypoints
    if not args.input_npz:
        video_name = args.viz_video
        keypoints = detector_2d(video_name)  # np.array of shape (N, 17, 2)
    else:
        npz = np.load(args.input_npz)
        keypoints = npz["kpts"]  # np.array of shape (N, 17, 2)

    keypoints_symmetry = metadata["keypoints_symmetry"]
    kps_left, kps_right = list(keypoints_symmetry[0]), list(keypoints_symmetry[1])
    joints_left, joints_right = list([4, 5, 6, 11, 12, 13]), list([1, 2, 3, 14, 15, 16])

    # normalize the keypoint coordinate to [-1, 1]
    keypoints = normalize_screen_coordinates(keypoints[..., :2], w=1000, h=1002)

    # create the 3D pose estimation model
    ...
    # the pose estimation model takes as input the keypoints only
    ...
    # np.array of shape (t, num_joint, 3)
    prediction = evaluate(gen, model_pos, return_predictions=True)

    # save 3D joint points
    np.save(
        f"outputs/{Path(args.video).stem}_3d_output.npy", prediction, allow_pickle=True
    )
...
def inference_video(args, detector_2d):
    """
    ...
    args.viz_video = args.video
    args.viz_output = f"{dir_name}/{args.detector_2d}_{video_name}.mp4"

    args.evaluate = "pretrained_h36m_detectron_coco.bin"

    with Timer(args.video):
        main(args)


if __name__ == "__main__":
    args = parse_args()
    inference_video(args, "alpha_pose")
